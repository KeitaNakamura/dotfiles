...
cd::narrow()
{
    ...
    local filter="$(__available "$ENHANCD_FILTER")"
    if [ "$(echo "$filter" | grep "fzf")" ]; then
        echo "$stdin" | eval $filter --filter=$1
    else
        ## ORIGINAL CODE
        local m="$(echo "$stdin" | awk 'tolower($0) ~ /\/.?'"$1"'[^\/]*$/{print $0}' 2>/dev/null)"
